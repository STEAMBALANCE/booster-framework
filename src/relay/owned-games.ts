...
export async function handleGetOwnedGames(msg: GetOwnedGamesRequest, bc: RelayPoster): Promise<void> {
  ...
  try {
    ...
    if (!msg.includePrices) {
      result = { games, pricesIncluded: false, ready };
    } else {
      ...
      result = { games: withPrice, pricesIncluded: true, currency, ready };
    }
  } catch {
    result = { games: [], pricesIncluded: !!msg.includePrices, ready: false };
  }
